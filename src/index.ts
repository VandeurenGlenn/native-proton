...
const tokenize = (
  key: string,
  value: string | number | BigNumber | object | []
) => {
...
const toType = (
  data: BigNumber | number | string | Uint8Array | ArrayBuffer | object | []
): Uint8Array => {
...
export const encode = (
  ...
  for (let i = 0; i < values.length; i++) {
    ...
    if (
      ...
      throw new Error(
        `minimumLength for ${token.key} is set to ${token.minimumLength} but got ${data.length}`
      );
...
export const decode = (
  ...
  if (keys.length !== deconcated.length)
    console.warn(
      `length mismatch: expected  ${keys.length} got ${uint8Array.length}`
    );
  ...
  for (let i = 0; i < values.length; i++) {
    ...
    if (isUint8Array(token.type)) output[token.key] = deconcated[i];
      ...
      output[token.key] = BigNumber.from(
        new TextDecoder().decode(deconcated[i])
      );
